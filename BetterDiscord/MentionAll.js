...
module.exports = new Plugin({
    name: 'Mention all',
    author: '#ŇƗŇĴΔ ŞΔŇ🐸#1337',
    description: 'Annoy people by mention @all, even if you do not have @everyone rights',
    color: '#FD0196',

    load: async() => {
        const { getGuild } = findModule('getGuild'), { getUser } = findModule('getCurrentUser'), { getGuildId } = findModule('getLastSelectedGuildId'), { getMembers } = findModule('getMembers')

        const m = EDApi.findModuleByDisplayName('ChannelEditorContainer').prototype
        monkeyPatch(m, 'render', {
            before: b => {
